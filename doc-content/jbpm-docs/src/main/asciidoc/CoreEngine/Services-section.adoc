...
By default when using query method of QueryService that accepts zero or more QueryParam instances (as we have seen in the previous examples) all of these params will be joined with AND operator meaning all of them must match.
...
* requires complex setup in cluster
